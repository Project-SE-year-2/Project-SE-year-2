...
CollisionConstraint checks that no single (program_id, date) cell has more
than K elective exam sessions scheduled.  Obligatory courses are invisible
...
Covers:
  ...
  - exactly K electives for a program on one day is satisfied (boundary)
  - K+1 electives for a program on one day is violated (boundary)
...
def test_k_plus_one_electives_on_one_day_is_violated():
    """K+1 elective exams for the same program on one day must fail."""
    constraint = CollisionConstraint(k=2)
    c1 = _elective_course("E1", "83101")
    c2 = _elective_course("E2", "83101")
...
def test_only_electives_trigger_violation():
    ...
    # Two electives on the same day → violation
    violating = _schedule(
        (e1, date(2026, 1, 5)),
        (e2, date(2026, 1, 5)),
    )
...
def test_different_programs_evaluated_independently():
    ...
    # B has 2 electives > K=1 → whole schedule fails
    assert constraint.is_satisfied(sched) is False

    # Remove B's extra elective: A=1, B=1 - both <= K=1
    sched.unassign(b2)
    assert constraint.is_satisfied(sched) is True
...
def test_year_does_not_isolate_elective_counts():
    ...
    sched = _schedule(
        # different year, same program → still 2 > K=1
...
def test_course_elective_in_one_program_obligatory_in_another():
    ...
    # Add e2: A now has 2 electives (> K=1) → fails; B still has 0 → not the cause
...
def test_k_zero_fails_when_any_elective_is_scheduled():
    """
    With k=0, even a single elective on any day violates the cap (count=1 > k=0).
    """
