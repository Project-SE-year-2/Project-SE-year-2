...
# Tests that no exams are scheduled outside the permitted date ranges
# for their respective periods.
def test_report_valid_dates_are_within_period_range(tmp_path):
    """
    Check that no exams in the generated output are scheduled
    outside the permitted date ranges.
    """
    course = Course("Physics 1", "83102", "Prof. A", "Exam")
    course.add_requirement(ProgramRequirement("83101", 1, "FALL", "Obligatory"))

    period = ExamPeriod("FALL", "Aleph", "01-02-2026", "05-02-2026")
    ...
    schedule = ExamSchedule(period)
    schedule.assign(course, date(2026, 2, 3))
    ...
    # Verify the assigned date is present
    assert "03-02-2026" in content

    # Ensure dates outside period range are not in the schedule section
    lines = content.split("\n")
    schedule_section = []
    in_schedule = False
    for line in lines:
        if "Complete Exam Schedules" in line:
            in_schedule = True
        if in_schedule:
            schedule_section.append(line)

    schedule_text = "\n".join(schedule_section)

    # Shouldn't contain dates outside the range within schedule section
    assert "31-01-2026" not in schedule_text
    assert "06-02-2026" not in schedule_text


# Tests that no two mandatory courses from the same academic year and study program
# are scheduled on the exact same date in the output.
def test_report_no_mandatory_course_clash_on_same_date(tmp_path):
    """
    Ensure the system never schedules two mandatory courses from the same
    academic year and study program on the exact same date in the output.
    """
    course1 = Course("Physics 1", "83102", "Prof. A", "Exam")
    course1.add_requirement(ProgramRequirement("83101", 1, "FALL", "Obligatory"))
    ...
    schedule.assign(course2, date(2026, 2, 2))
    ...
    for line in lines:
        if "Complete Exam Schedules" in line:
            schedule_section_started = True
        if schedule_section_started:
            # Parse exam dates from lines like "01-02-2026: Physics 1 (83102) Prof. A"
            if any(course.course_id in line for course in [course1, course2]):
                # Extract date and course
                parts = line.split(":")
                if len(parts) >= 1 and "-" in parts[0]:
                    try:
                        exam_date = parts[0].strip()
                        course_id = course1.course_id if course1.course_id in line else course2.course_id
                        if exam_date not in exam_dates:
                            exam_dates[exam_date] = []
                        exam_dates[exam_date].append(course_id)
                    except:
                        pass

    # Verify no two mandatory courses from same program are on same date
    for date_str, course_ids in exam_dates.items():
        if len(course_ids) > 1:
            # Multiple courses on same date should not be both mandatory from same program
            pytest.fail(
                f"Two mandatory courses from same program scheduled on same date: "
                f"{date_str} -> {course_ids}"
            )
